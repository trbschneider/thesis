...
It should be noted that in all our \gls{QSI} studies presented here, the \gls{SGP} is heavily violated, which, technically speaking, prohibits the application of the {\q}-space formalism as defined by \citet{Callaghan:1996} (see e.g. \citep{King:1994}). The term \gls{QSI} is used here to describe the analysis pipeline associated with the {\q}-space formalism, disregarding the known \gls{SGP} violation. The theoretical and practical implications of this are discussed in general in Chapter~\ref{chapter2} and specifically for this study at the end of this chapter and the following Chapter~\ref{chapter6}.
\section{Methods}
