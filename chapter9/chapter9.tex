...
The increased spatial resolution here provides a large number of voxels that are completely contained in the CC. As a consequence, the CC can be easily distinguished from surrounding tissue and parameter estimates are less influenced by CSF contamination, particularly in the thinning part of the CC (B3--S1). This is advantageous especially for subjects with smaller CC such as in s2 \& s3, but is also important in view of future applications in patientss with neurological diseases such as MS or Alzheimers, which often suffer from CC atrophy. 
