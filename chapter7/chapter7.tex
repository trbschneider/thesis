...
\begin{description}
	\item[\OI:] The orientation invariant protocol optimisation as described in \citep{Alexander:2008}
...
\item[\SD:] Similar to the {\gls{OI}} optimisation approach, the algorithm uses a fixed gradient scheme for each of the $M$ shells and only optimises the PGSE pulse parameters. The main difference to \gls{OI}{} is that the algorithm optimises for a single a-priori known fibre direction instead of a set of \lq worst case\rq{} fibre directions.
...
\begin{table}[H]
\caption{Protocol parameters and gradient schemes for $N=90$, $M=8$. The $M$ different gradient direction schemes are coded by different colors. Please note that in the \gls{OI}{} and \SD{} protocols all $M$ gradient direction schemes are identical.}
\label{fig:chapter7 exp1 protocol 60mT}
\begin{adjustbox}{width=0.9\linewidth,keepaspectratio,minipage=\linewidth}
\tiny
\subfloat[\gls{OI} protocol]
{
...
The simulation results show that the combination of stronger diffusion weighting and custom gradient scheme in the \FD{} design is most successful in estimating the cylinder radius, especially when $R$ is very small.
...
}
{
    ...
    \begin{figure}
    	[H] \centering \input{chapter7/figs/protocols_par1/OInv_62mT.tex} \caption{\gls{OI} protocol optimised for clinical gradient strength.}
